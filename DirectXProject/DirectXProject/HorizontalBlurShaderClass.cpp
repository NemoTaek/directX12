...
bool HorizontalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	//return result;
	return InitializeShader(device, hwnd, L"./HorizontalBlur.vs", L"./HorizontalBlur.ps");
}
...
bool HorizontalBlurShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	...
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, screenWidth))	return false;
...
bool HorizontalBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	// 텍스처 이동 버퍼 생성
	if (FAILED(device->CreateBuffer(&screenSizeBufferDesc, NULL, &m_screenSizeBuffer)))	return false;
...
bool HorizontalBlurShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	...
	if (FAILED(deviceContext->Map(m_screenSizeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource)))	return false;
	ScreenSizeBufferType* screenSizePtr = (ScreenSizeBufferType*)mappedResource.pData;
