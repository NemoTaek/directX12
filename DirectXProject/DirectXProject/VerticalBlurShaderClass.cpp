...
bool VerticalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	//return result;
	return InitializeShader(device, hwnd, L"./VerticalBlur.vs", L"./VerticalBlur.ps");
}
...
bool VerticalBlurShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenHeight)
{
	...
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, screenHeight))	return false;
