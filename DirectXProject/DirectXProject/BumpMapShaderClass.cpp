...
bool BumpMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	//return result;
	return InitializeShader(device, hwnd, L"./BumpMap.vs", L"./BumpMap.ps");
}
...
bool BumpMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, NULL, NULL, "BumpMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage))) {
	...
	if (FAILED(D3DCompileFromFile(psFilename, NULL, NULL, "BumpMapPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage))) {
